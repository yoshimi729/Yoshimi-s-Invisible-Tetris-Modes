...
PhantomPracticeGame.name = "Phantom Practice Appearing"
PhantomPracticeGame.hash = "PhantomPracticeAppearing"
PhantomPracticeGame.tagline = "Your stack gradually becomes visible! Blocks stop appearing at level 900."
...
function PhantomPracticeGame:new()
	...
	self.invis_lock = 0 -- stops new pieces from appearing once level 900 is reached
end
...
function PhantomPracticeGame:advanceOneFrame()
	...
	if self.level > 899 then
		self.invis_lock = self.invis_lock + 1
	end
	return true
end
...
function PhantomPracticeGame:drawGrid()
	PhantomPracticeGame.rollOpacityFunction = function(age)
		local visibility = 60 + math.max(0, (self.level / 2) - 60) + self.invis_lock
		if age < visibility then return 0
		else return 1 - visibility / age end
	end
